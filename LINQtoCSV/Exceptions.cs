...
namespace LINQtoCSV
{
    ...
    /// <summary>
    /// Thrown when a field label in the type could not be found in the input
    /// </summary>
    [Serializable]
    public class MissingFieldNameException : LINQtoCSVException
    {
        public MissingFieldNameException(string fieldName) :
            base(string.Format("A column labelled {0} was expected in the file, but was not found.", fieldName))
        {
            Data["FieldName"] = fieldName;
        }
    }
    ...
    public class AggregatedException : LINQtoCSVException
    {
        ...
        public void AddException(Exception e)
        {
            ...
            {
                foreach (var inner in ((AggregatedException)e).m_InnerExceptionsList)
                {
