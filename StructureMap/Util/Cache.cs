...
namespace StructureMap.Util
{
    ...
    public class Cache<KEY, VALUE> : IEnumerable<VALUE> where VALUE : class
    {
        ...
        private Dictionary<KEY, VALUE> _values;
        ...
        {
            _locker = new object();
            _getKey = delegate { throw new NotImplementedException(); };
            _values = new Dictionary<KEY, VALUE>();
            _onMissing = delegate(KEY key)
            {
        ...
        public Cache(Dictionary<KEY, VALUE> dictionary, Func<KEY, VALUE> onMissing)
        {
            _locker = new object();
            _getKey = delegate { throw new NotImplementedException(); };
            _onMissing = onMissing;
        ...
        public Cache(Dictionary<KEY, VALUE> dictionary)
        {
            _locker = new object();
            _getKey = delegate { throw new NotImplementedException(); };
            _values = dictionary;
