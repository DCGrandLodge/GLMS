...
CREATE PROCEDURE [dbo].ELMAH_SetResolved
(
    @Application NVARCHAR(60),
    @ErrorId UNIQUEIDENTIFIER,
	@Resolved bit
) AS
  UPDATE [Elmah_Error] set Resolved = @Resolved where Application = @Application and ErrorId=@ErrorId
  GO
...
AS 
    ...
    BEGIN
        ...
        WHERE   
            [Application] = @Application
			and (Resolved = 0 or Resolved = cast(@IncludeResolved as int))
        ORDER BY 
            [TimeUtc] DESC, 
            [Sequence] DESC
	...
	AND
	    (Resolved = 0 or Resolved = cast(@IncludeResolved as int))
    ORDER BY
        [TimeUtc] DESC, 
        [Sequence] DESC
    FOR
