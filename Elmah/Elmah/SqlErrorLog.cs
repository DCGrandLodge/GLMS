...
namespace Elmah
{
    ...
    public class SqlErrorLog : ErrorLog
    {
        ...
        public override int GetErrors(int pageIndex, int pageSize, IList errorEntryList, bool includeResolved = true)
        {
            if (pageIndex < 0)
            ...
            using (SqlConnection connection = new SqlConnection(this.ConnectionString))
            using (SqlCommand command = Commands.GetErrorsXml(this.ApplicationName, pageIndex, pageSize, includeResolved))
            {
                command.Connection = connection;
                connection.Open();
            ...
            AsyncCallback asyncCallback, object asyncState)
        {
            ...
            SqlCommand command = Commands.GetErrorsXml(this.ApplicationName, pageIndex, pageSize, true);
            command.Connection = connection;
        ...
        public override void DeleteErrors(DateTime? olderThan)
        {
            using (SqlConnection connection = new SqlConnection(this.ConnectionString))
            using (SqlCommand command = new SqlCommand())
            {
                ...
                {
                    command.CommandText = "delete from ELMAH_Error where TimeUtc < @olderThan";
                    command.Parameters.AddWithValue("@olderThan", olderThan.Value.ToUniversalTime());
                }
                else
                {
                    command.CommandText = "truncate ELMAH_Error";
                }
        ...
        public override void ResolveError(Guid errorId, bool resolved)
        {
            using (SqlConnection connection = new SqlConnection(this.ConnectionString))
            using (SqlCommand command = Commands.SetResolved(this.ApplicationName, errorId, resolved))
            {
                command.Connection = connection;
                connection.Open();
                command.ExecuteNonQuery();
        ...
        public override string GetErrorIDBySequence(int sequence)
        {
            using (SqlConnection connection = new SqlConnection(this.ConnectionString))
            using (SqlCommand command = new SqlCommand())
            {
                command.Connection = connection;
                command.CommandText = "select ErrorId from ELMAH_Error where Sequence=@seq";
        ...
        public override ErrorLogEntry GetError(string id)
        {
            ...
            using (SqlCommand command = Commands.GetErrorXml(this.ApplicationName, errorGuid))
            {
                ...
                {
                    reader.Read();
                    errorXml = reader.GetString(0);
                    resolved = reader.GetBoolean(1);
                }
            }
        ...
        private sealed class Commands
        {
            ...
            public static SqlCommand SetResolved(string appName, Guid id, bool resolved)
            {
                SqlCommand command = new SqlCommand("ELMAH_SetResolved");
                command.CommandType = CommandType.StoredProcedure;

                SqlParameterCollection parameters = command.Parameters;
                parameters.Add("@Application", SqlDbType.NVarChar, _maxAppNameLength).Value = appName;
                parameters.Add("@ErrorId", SqlDbType.UniqueIdentifier).Value = id;
                parameters.Add("@Resolved", SqlDbType.Bit).Value = resolved;
                return command;
            }
            ...
            public static SqlCommand GetErrorsXml(string appName, int pageIndex, int pageSize, bool includeResolved)
            {
                ...
                parameters.Add("@IncludeResolved", SqlDbType.Bit).Value = includeResolved;
