...
namespace Elmah
{
    ...
    public sealed class Error : ICloneable
    {
        ...
        private NameValueCollection _entityValidationErrors;
        ...
        public Error(Exception e, HttpContext context)
        {
            ...
            {
                var re = ((ReflectionTypeLoadException)e);
                _detail += "\n\n" + String.Join("\n\n", re.LoaderExceptions.ToList().Select(x => x.Message));
            }
            if (e is DbEntityValidationException)
            {
                _entityValidationErrors = new NameValueCollection();
                foreach (var val in ((DbEntityValidationException)e).EntityValidationErrors)
                {
                    string entityType = val.Entry.Entity.GetType().Name;
                    foreach (var err in val.ValidationErrors)
                    {
                        _entityValidationErrors.Add(String.Format("{0}.{1}", entityType, err.PropertyName), err.ErrorMessage);
                    }
                }
            }
        }
        ...
        /// <summary>
        /// Gets or sets the resolved status of the error.
        /// </summary>
        ...
        {
            get { return _sequence ?? String.Empty; }
            set { _sequence = value; }
        }
        ...
        /// <summary>
        /// Gets a collection representing the Entity Validation errors 
        /// captured as part of diagnostic data for the error.
        /// </summary>

        public NameValueCollection EntityValidationErrors
        {
            get { return FaultIn(ref _entityValidationErrors); }
        }
        ...
        object ICloneable.Clone()
        {
            ...
            copy._entityValidationErrors = CopyCollection(_entityValidationErrors);
