...
[assembly: Elmah.Scc("$Id: ErrorJson.cs 566 2009-05-11 10:37:10Z azizatif $")]
...
namespace Elmah
{
    ...
    public sealed class ErrorJson
    {
        ...
        public static string EncodeString(ErrorLogEntry logEntry)
        {
            if (logEntry == null) 
                throw new ArgumentNullException("error");

            StringWriter writer = new StringWriter();
            Encode(logEntry, writer);
        ...
        public static void Encode(ErrorLogEntry logEntry, TextWriter writer)
        {
            if (logEntry == null) 
                throw new ArgumentNullException("error");
            ...
            EncodeEnclosed(logEntry, new JsonTextWriter(writer));
        }

        private static void EncodeEnclosed(ErrorLogEntry logEntry, JsonTextWriter writer)
        {
        ...
        internal static void Encode(ErrorLogEntry logEntry, JsonTextWriter writer)
        {
            Debug.Assert(logEntry!= null);
            Debug.Assert(writer != null);
            var error = logEntry.Error;
            Member(writer, "id", logEntry.Id);
            Member(writer, "application", error.ApplicationName);
            Member(writer, "sequence", error.Sequence);
            ...
            Member(writer, "entityValidationErrors", error.EntityValidationErrors);
